...
### Changed
- 飞书回复卡中的图片附件现在显示在附件块声明的位置：块写在正文中间，图片就随文出现在那里，不再统一排在正文末尾；其他类型文件维持原样（回复之后单独发送）。
- 状态卡片标题中的错误原因从 30 字符放宽到 48 字符，截断后能看到更多病因（完整错误仍在日志与终态卡片）。

### Fixed
- 多处模型提供方错误提示文案清理：超时不再显示成 "Timeout: timed out"，网络错误跳过 "error sending request for url" 套话直接呈现最底层病因，重试失败标题不再重复 "Request failed"，Api 错误不再带 Some() 调试噪音。

## [0.10.42] - 2026-09-25

### Added
- GUI 安装包现在内嵌同版本 CLI 并在启动时把它加进 PATH；用 Homebrew cask 装 GUI 会自动连带安装 CLI。

## [0.10.41] - 2026-09-25

### Added
- 飞书回复中声明的图片附件现在随回复卡片一起展示，不再在回复之后单独发送图片消息。
- 新增 INSTALL.md 安装与排障文档，写明 GUI 须连带安装 CLI（agent 依赖 PATH 上的 yomi 命令）。

### Fixed
- `yomi session wait` 等待期间会话被删除时现在以退出码 4 明确结束；`yomi cron update` 只给 `--work-dir` 不给对象时现在显式报错而非静默忽略。

## [0.10.40] - 2026-09-24

### Fixed
- Windows：GUI 与后台服务启动时不再弹出黑色控制台窗口。

### Changed
- GUI 的 Quote 浮钮改为纯图标样式（去掉文字标签）。

## [0.10.39] - 2026-09-24

### Added
- 新增旁问能力：agent 长任务运行期间，可用 `yomi btw`（CLI）或 GUI 旁问浮卡另行提问，不排队、不打断正在进行的任务。
...
- 命令与脚本可以把图片直接交给模型：输出中独占一行的 `yomi://image/<路径>` 标记即随结果回传图片，无需再手动读取文件；相对路径按命令工作目录解析，每次最多 10 张，支持 png/jpeg/gif/webp。

### Fixed

- 修复工具结果中的图片送达不到模型的问题：此前模型实际看不到 read 工具读取的图片（按通道不同被静默丢弃或导致请求报错）。
- 头信息完好但内容已损坏的图片不再原样发给模型，避免整轮请求被接口拒绝、会话卡死。
- 单次请求转发的历史图片数量增设上限（50 张），超出时最旧的图片替换为文字占位说明，会话不再因图片累积被接口拒绝。
...
- 修复 macOS 无输入法时在输入框内容末尾按方向键仍会插入控制字符的问题（上一版只拦住了输入法路径的乱码方块字符）。
- 修复代码块里的搜索高亮在语法着色完成后丢失、且关闭搜索栏后可能残留擦不掉的问题；搜索打开期间高亮现在会持续钉在最新内容上。

## [0.10.21] - 2026-09-05

### Fixed

- 修复 GUI 聊天搜索高亮可能残缺（只高亮出上一次输入的片段）、更换搜索词后旧词高亮不消失、关闭搜索栏后高亮仍残留的问题。
- 修复 macOS 输入法下在输入框内容末尾按方向键可能输入乱码方块字符的问题；乱码防护现覆盖所有文本输入框（含搜索框，此前仅主输入框）。
- 修复聊天搜索在流式输出进行中时，当前匹配的高亮可能停在已失效位置的问题。

## [0.10.20] - 2026-09-05

### Changed

- 频道回复的正文选取从固定取最后两段改为：最后一段始终在正文里，仅当更早有比它更长的正文时把最长的那段拼在前面（并列等长则不拼）——重要结论即使落在倒数第三段等更早位置也会进入正文；文档评论的回复同样适用。
- 最后一段已是本轮最长正文时，回复正文只保留它一段，较短的开场叙述改收在折叠过程面板里。
- 两段正文共享长度预算时，较短一段用不完的部分会分给较长一段，长正文被截断的情况减少。
- 回复中 @ 了人的内容即使收在折叠过程面板里，也会以新消息发送回复以保证对方收到提醒。
...
- GIF 动图读取改为只取第一帧，并附帧数、时长与抽帧命令提示，不再整段内联——此前几 MB 的动图会把请求体撑爆、被上游以 HTTP 413 拒绝，且该回合之后每次发问都重蹈覆辙；飞书/Telegram 收到的动图同样处理，需要看完整动画可让 agent 用命令行抽帧。
- 图片解码失败、文件超过大小上限时，返回带文件基本信息的明确「不支持」说明，不再是笼统的读取失败。

### Changed

- 读取工具按内容而非扩展名识别图片，扩展名写错也能正常读图；视频、压缩包等二进制文件不再被当文本读出（此前会把乱码灌进对话），改为返回「不支持」说明并提示用命令行工具查看。
...
- 模型偶发返回空回复（无正文、无工具调用，例如上游抖动）时不再把空消息写进会话记录：此前这条空消息会随每次后续请求重放，被网关以 400「assistant 消息不能为空」拒绝，会话从那一刻起永久卡死、只能手工编辑日志救回；现在该回合以报错干净收场，重发消息即可继续。
- 已被空消息卡死的存量会话升级后自动康复：每轮发问前的记录清理会摘除这类空消息，无需手工处理。
...
- 新增 `yomi config schema` 子命令：输出当前版本配置的 JSON Schema（由代码生成、与代码始终一致），可挂到编辑器获得 config.toml 的字段补全与校验。

### Changed

- 飞书终态回复卡现在展示一次 run 的全部中间正文：最终答复下方新增默认展开的过程面板，按时间顺序完整呈现每段中间正文（此前只保留 120 字摘要且默认折叠）。
- 过程面板中连续的工具调用就地折叠为小面板，标题直接显示工具名与总耗时（如 `🔧 shell×2 · read · 1m05s`），点开查看每条明细；只有工具调用、没有中间正文的 run 卡片保持原样。
- 异常冻结的终态收据卡与回复卡统一为同一样式面板（默认折叠），中间正文同样完整保留。
- 纯文本平台（Telegram 等）的回复在最终文本后附带按时间序排列的中间正文全文，不再只有工具摘要行。
- 卡片正文渲染前自动补齐因中断或截断而未闭合的代码块围栏，避免整块内容退化为原始标记文本。

### Removed

- 删除无消费方的模型配置项 `fallback_model_id`（此前设置后不产生任何效果）。
...
- 按会话的规则文件 `<数据目录>/sessions/rules/<session_id>.md` 不再读取（由上面的群规则取代）；fork 会话不再复制规则文件。

### Changed

- watch 观察者注入 system prompt 的契约精简为三句（watch 模式事实 / 输出不可见 / 需要时经 skill 发言），不再脚本化应答策略，也不含 skill 与锚点的操作提示（材料本就在 prompt 与消息头里）。
...
- daemon socket 支持密码鉴权（仅 ws/wss 传输）：daemon 侧在 config.toml 设置 `socket_auth_hash`（blake3 哈希，可用新命令 `yomi daemon auth-hash` 生成；也可用环境变量 `YOMI_SOCKET_AUTH_HASH` 覆盖），客户端设置 `YOMI_SOCKET_AUTH` 为明文密码即可自动完成握手鉴权；未携带或密码错误的连接在 WebSocket 握手阶段即被拒绝（HTTP 401）。unix socket 不受影响，仍由文件权限保护。
- ws 握手鉴权失败后服务端自我节流 300ms：在线爆破速率被封顶在每秒约 3 次（与攻击并行度无关）。
...
- 新增 `set_channel_watch` RPC：`on` 缺省为查询该群 watch 状态（Vim `:set` 风格），传 `true`/`false` 开关；开启会自动创建（或带记忆复任）该群的观察者会话并返回其 session_id。
