...
{#if stats.actionCount > 0}
  <div class="overflow-hidden rounded-md border border-border/60 bg-background">
    <button
      type="button"
      class="flex min-h-8 w-full items-center gap-2 px-2.5 text-left transition-colors hover:bg-secondary/30 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-ring"
    ...
    >
      <ChevronDown
        class="size-3.5 shrink-0 text-muted-foreground transition-transform duration-150 {expanded
          ...
          : '-rotate-90'}"
      />
      <span class="shrink-0 text-xs font-medium text-foreground">Activity</span>
    ...
    {#if expanded}
      <div class="border-t border-border/60 bg-secondary/10 px-2.5 py-1.5">
