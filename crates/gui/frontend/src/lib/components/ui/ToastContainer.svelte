<script lang="ts">
  ...
  const colorMap = {
    ...
    success: "border-emerald-500/20 bg-emerald-50 dark:bg-emerald-950 text-emerald-700 dark:text-emerald-300",
    warning: "border-amber-500/20 bg-amber-50 dark:bg-amber-950 text-amber-700 dark:text-amber-300",
    error: "border-red-500/20 bg-red-50 dark:bg-red-950 text-red-700 dark:text-red-300",
  };
...
<div class="fixed top-4 left-1/2 -translate-x-1/2 z-[100] flex flex-col gap-2 max-w-sm w-full pointer-events-none">
  {#each toasts as toast (toast.id)}
    ...
    <div
      class="pointer-events-auto flex items-start gap-2.5 rounded-lg border px-3.5 py-2.5 shadow-lg transition-all duration-300 {colorMap[toast.type]}"
    >
