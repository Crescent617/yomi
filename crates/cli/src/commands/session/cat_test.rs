...
fn user_and_assistant_text_only() {
    ...
    let out = format_transcript(messages, &data_dir());
...
fn no_displayable_messages_returns_empty() {
    ...
    assert!(format_transcript(messages, &data_dir()).is_empty());
}
...
fn steer_message_is_labeled() {
    ...
    let out = format_transcript(vec![msg], &data_dir());
...
fn tool_result_image_resolves_to_real_path() {
    ...
    let out = format_transcript(vec![assistant, tool_msg], &data_dir());
...
fn dangling_tool_call_is_shown_as_interrupted() {
    ...
    let out = format_transcript(vec![assistant], &data_dir());
    assert!(out.contains("=== assistant · "));
