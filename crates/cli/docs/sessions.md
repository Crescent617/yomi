...
- `yomi session wait [-s <id>]`：轮询至静默（`phase=idle`、无 running 子 agent、无后台 shell）后 exit 0；首个探测失败（会话不存在/daemon 不在）exit 2；`--timeout <秒>` 超时 exit 3（默认永不超时）。`yomi session send` + `yomi session wait` = 驱动兄弟会话的最小回路。
