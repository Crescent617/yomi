...
/// The chat-scoped rules file shared by every session of one chat
/// (chat-level, thread, doc-comment, watch observer alike):
/// `<data_dir>/channels/rules/<chat_id>.md`. Only `[A-Za-z0-9_-]` ids
/// may name a rules file — an id like `../../etc/x` would make the
/// daemon read an arbitrary `.md` into the system prompt.
pub(crate) fn channel_rules_path(
    data_dir: &std::path::Path,
    chat_id: &str,
) -> Option<std::path::PathBuf> {
    valid_rules_id(chat_id).then(|| {
        data_dir
            .join("channels")
            .join("rules")
...
async fn read_rules_file(path: &std::path::Path) -> Option<String> {
    ...
    let file = match tokio::fs::File::open(path).await {
        ...
        Err(e) => {
            ...
            tracing::warn!(path = %path.display(), "cannot open channel rules: {e}");
            return None;
        }
    };
    let mut buf = Vec::new();
    if let Err(e) = file
        ...
        .await
    {
        tracing::warn!(path = %path.display(), "cannot read channel rules: {e}");
        return None;
    }
...
/// Read the chat's rules for prompt injection: verbatim, capped, no
/// framing — the file speaks for itself. Missing/empty → `None` (zero
/// prompt noise).
pub(crate) async fn channel_rules_section(
    data_dir: &std::path::Path,
    chat_id: &str,
) -> Option<String> {
