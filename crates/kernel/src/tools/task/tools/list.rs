use crate::tools::task::store::SharedTaskStore;
use crate::tools::task::types::{ListTasksOutput, TaskListItem};
...
impl Tool for TaskListTool {
    ...
    async fn exec(&self, args: Value, _ctx: ToolExecCtx<'_>) -> Result<ToolOutput> {
        ...
        let resolved_ids: std::collections::HashSet<_> = all_tasks
            .iter()
            .filter(|t| matches!(t.status, crate::tools::task::types::TaskStatus::Completed))
            .map(|t| t.id.clone())
            .collect();
