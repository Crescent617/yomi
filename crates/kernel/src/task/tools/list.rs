...
impl Tool for TaskListTool {
    ...
    async fn exec(&self, _args: Value) -> Result<ToolOutput> {
        let task_list_id = self.get_task_list_id();
        let all_tasks = self.store.list_tasks(&task_list_id).await?;

        let resolved_ids: std::collections::HashSet<_> = all_tasks.iter()
            .filter(|t| matches!(t.status, crate::task::types::TaskStatus::Completed))
            .map(|t| t.id.clone())
            .collect();

        let tasks: Vec<TaskListItem> = all_tasks.into_iter()
            .filter(|t| !t.metadata.as_ref().map(|m| m.contains_key("_internal")).unwrap_or(false))
            .map(|task| TaskListItem {
                id: task.id,
                subject: task.subject,
                status: task.status,
                owner: task.owner,
                blocked_by: task.blocked_by.into_iter()
