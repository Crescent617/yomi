...
| `--daemon` / `--local` | 强制 daemon（不存在则 spawn，同 TUI）/ 强制本地 |
| `--ephemeral` | 不把 session 记为当前目录的 last session |
| `-v, --verbose` | 工具调用/重试进度打到 stderr（stdout 只放结果） |

## Kernel 选择（默认 auto）

- **默认 auto**：`try_connect()` 探测 daemon，活着就用 daemon（可与其他客户端共享会话、可观测），不在则本地 in-process kernel。**auto 模式不 spawn daemon**（CI 场景不留后台进程）。
- `--daemon`：强制 daemon，不存在则 spawn（同 TUI `--daemon`）。
- `--local`：强制本地。
